...
let glitchAmount = map(
  ...
  10
    ...
    if (frameCount % random(10, 20) === 0) {
        ...
        let c = random(bluePalette);

fill(red(c), green(c), blue(c), random(50,100));
    ...
    for (let i = 0; i < 10; i++) {
     ...
     fill(random(bluePalette));   // Random blue value
        
        fill(random(bluePalette));              // BluePalette mode
    ...
    if (frameCount % 60 === 0) {
        ...
        let glitchText = ['GoodNight', 'Bye', 'ERROR', 'LIGHT ', 'Hi'][floor(random(0, 5))];
        fill(255, 255, 255);    // text colour white
        textSize(height/4);          // text size big!
        for(let i = 0; i < glitchText.length; i++){

  let charX = width/2 - 200 + i * 80;
  ...
  text(
    ...
    for (let y = frameCount % 10; y < height; y += 9) {
    ...
    for (let i = 0; i < 15; i++) {
        ...
        let hue = random(180, 240);  // Random color
    ...
    for (let i = 0; i < 400; i++) {
        ...
        let hue = random(180, 240); // Random hue for colorful noise
        let sat = random(50, 100); // Random saturation for vibrant colors
        let bri = random(60, 100); // Random brightness for visible dots
